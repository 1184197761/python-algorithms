...
def fitCal(index, link):
    if len(set(pop[index])) == NF:
        return 1/link
    else:
        return 0
...
for iter in range(MI):  # 种群迭代
    ...
    minLink, maxLink = links[index1], links[index2]
    best = pop[index1]  # 最佳适应性个体
    screenPop()  # 筛选种群个体
    crossPop()  # 种群个体交配
    mutatePop()  # 种群个体变异
    pop[index2] = best  # 淘汰最差并保留最佳适应性个体
    if not iter % (MI/10) or minLink == 3:  # 迭代进度达到设置的10%或最优结果
        print('迭代次数：%3d\t时间：%.2fs\t板间连线：%2d' %
              (iter+1, time()-tstart, minLink))
        if minLink == 3:  # 迭代达到最少板间连线结果
            break  # 停止迭代
print('最少连线：%3d\t适应值：%.1f\t分配关系：' % (minLink, maxFit))
writeResult()
