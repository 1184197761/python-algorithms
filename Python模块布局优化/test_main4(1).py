    ...
    for module in modules.values():  # 遍历模块值列表
        ...
        if mirror:  # 若镜像量非零
            ...
            for port in module.ports:
                port.bound = [[2*center[0]-x, y]
                              for [x, y] in port.bound]  # 镜像端口
        if angle:  # 若旋转量非零
...
def comparerotate(Mirror, Angle, Mirror_former, Angle_former):
    Mirror_latter = Mirror    # 更新模块的镜像量
    Angle_latter = Angle  # 更新模块的旋转量

    for module in modules.values():
        if (Mirror_former[module.id-1] == 0) & (Mirror_latter[module.id-1] == 0):   # 更新前后均未镜像
            Mirror[module.id - 1] = 0  # 无镜像
            Angle[module.id - 1] = Angle_former[module.id-1] + \
                Angle_latter[module.id-1]  # 前后两次旋转量叠加

        # 更新前无镜像，更新后镜像
        elif (Mirror_former[module.id-1] == 0) & (Mirror_latter[module.id-1] == 1):
            Mirror[module.id - 1] = 1  # 镜像
            Angle[module.id - 1] = 4 - Angle_former[module.id-1] + \
                Angle_latter[module.id-1]
            # 旋转angle后镜像，相当于镜像后旋转360-angle，即4-angle。再加上镜像后的旋转量

        # 更新前镜像，更新后不改变，仍镜像
        elif (Mirror_former[module.id-1] == 1) & (Mirror_latter[module.id-1] == 0):
            Mirror[module.id - 1] = 1  # 仍镜像
            Angle[module.id - 1] = Angle_former[module.id-1] + \
                Angle_latter[module.id-1]  # 前后两次旋转量叠加

        # 更新前镜像，更新后再镜像，变为无镜像
        elif (Mirror_former[module.id-1] == 1) & (Mirror_latter[module.id-1] == 1):
            Mirror[module.id-1] = 0     # 镜像两次则无镜像
            Angle[module.id - 1] = 4 - Angle_former[module.id - 1] + \
                Angle_latter[module.id - 1]
            # 旋转angle后镜像，相当于镜像后旋转360-angle，即4-angle。再加上镜像后的旋转量

        if Angle[module.id - 1] >= 4:   # 前后旋转量超过360°，减去360°，归一化至0~360°
            Angle[module.id-1] = Angle[module.id-1] - 4
...
def saveresult(Mirror, Angle, Center):  # 保存更新模块结果
    ...
    for module_num in sortmodule:  # 遍历模块序号
        ...
        file.write("\n")
        file.write("Offset:" + "[%3.3f, %3.3f]" %
                   (Center[module_num-1][0], Center[module_num-1][1]))
        file.write("\n")
        file.write("\n")
...
if __name__ == '__main__':
    ...
    Mirror = np.zeros(len(modules))             # 初始化所有模块的镜像量
    Angle = np.zeros(len(modules))              # 初始化所有模块的旋转量
    Center = np.zeros((len(modules), 2))        # 初始化所有模块的中心点

    Mirror_former = np.zeros(len(modules))      # 最初所有模块的镜像量
    Angle_former = np.zeros(len(modules))       # 最初所有模块的旋转量
    ...
    while iter <= MAXITER:
        ...
        # 对比最初的旋转量和镜像量
        [Mirror, Angle] = comparerotate(
            Mirror, Angle, Mirror_former, Angle_former)
        Mirror_former = copy.deepcopy(Mirror)
        Angle_former = copy.deepcopy(Angle)
