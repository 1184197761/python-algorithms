...
def updateMods():  # 更新模块
    for module in modules.values():  # 遍历模块值列表
        ...
        if mirror:  # 若镜像量非零
            module.bound = [2*center-vertex for vertex in module.bound]  # 镜像模块
            for port in module.ports:
                port.bound = [2*center-vertex for vertex in port.bound]  # 镜像端口
        if angle:  # 若旋转量非零
...
if __name__ == '__main__':
    ...
    orgArea = plotRect()  # 初始版图面积
    plotMods()  # 绘制模块
    plotLinks()  # 绘制线网
    updateMods()  # 更新模块
    checkOverlap()  # 检查重叠
    axes = figure.add_subplot(122, aspect=1)  # 子图2
    newArea = plotRect()  # 最终版图面积
    plotMods()
    plotLinks()
