...
def allocation():
    ...
    gene2 = [np.random.randint(0, 4) for _ in modules]
    return [gene1, gene2]


def areaCal(ind):
    updateMods(ind[0], ind[1])
    vertexs = [vertex for module in modules.values()
               for vertex in module.bound]  # 获取所有模块顶点列表
    vertexs = np.array(vertexs)  # 顶点列表转np数组
...
def screenPop():  # 筛选种群个体
    sumFit = np.cumsum(fitness)  # 计算累加适应值列表（生成轮盘）
    for ind1 in pop:  # 遍历每个个体
        for index, ind2 in enumerate(pop):  # 遍历新的个体（轮盘转圈）
            if np.random.random() < sumFit[index]:  # 若轮盘选中个体适应值较大
...
def crossPop():  # 种群个体交配
    for i in range(int(NP/2)):  # 遍历雄性个体
        j = np.random.randint(int(NP/2), NP-1)  # 寻找配偶
        if np.random.random() < PC:  # 求偶成功
            # index = np.random.randint(1, len(modules))  # 染色体交叉点
            # child1 = pop[i][1][:index]+pop[j][1][index:]  # 子染色体
            # child2 = pop[j][1][:index]+pop[i][1][index:]  # 或子代个体
            # pop[i][1], pop[j][1] = child1, child2  # 牺牲父代，保留子代
            rands = np.random.choice(16, 2)
            index1, index2 = sorted(rands)
            child1 = pop[i][0]+pop[j][0][index1:index2]
            child2 = pop[j][0]+pop[i][0][index1:index2]
            for child in [child1, child2]:
                for gene in child:
                    if child.count(gene) > 1:
                        child.remove(gene)
            pop[i][0], pop[j][0] = child1, child2


def mutatePop():  # 种群个体变异
    for ind in pop:
        if np.random.random() < PW:  # 触发变异
            np.random.shuffle(ind[0])  # 超级变异
