...
def updateMods():  # 更新模块
    ...
    levels = [[0, 0, 500]]
    minDex = 0
    minLevel = levels[minDex]
    for index1, module in enumerate(modules.values()):
        if module.moved:
            continue
        tempDex = minDex
        while(tempDex == minDex):
            for module in list(modules.values())[index1:]:
                if module.moved:
                    continue
                vertexs = np.array(module.bound)
                vertex1, vertex2 = vertexs.min(
                    axis=0), vertexs.max(axis=0)  # 获取对角坐标
                width, height = vertex2-vertex1  # 获取矩形宽高
                if minLevel[2]-minLevel[0] > width:
                    shift = minLevel[0:2]-vertex1
                    module.bound = [vertex+shift for vertex in module.bound]
                    for port in module.ports:
                        for vertex in port:
                            vertex += shift   # 移动端口
                    module.moved = True
                    newLevel = [minLevel[0], minLevel[1] +
                                height, minLevel[0]+width]
                    minLevel[0] += width
                    levels.insert(minDex, newLevel)
                    minDex += 1
                    break
                elif minLevel[2]-minLevel[0] > height:
                    center = np.mean(module.bound, 0)  # 中心点
                    angle = 1
                    module.bound = [rotateVertex(vertex, center, angle)
                                    for vertex in module.bound]  # 旋转模块
            ...
            if minDex == 0:
                nearDex = 1
            elif minDex == len(levels)-1:
                nearDex = minDex-1
            else:
                leftDex = minDex-1
                rightDex = minDex+1
                nearDex = leftDex if levels[leftDex][1] < levels[rightDex][1] else rightDex
            if nearDex < minDex:
                levels[nearDex][2] = minLevel[2]
            elif nearDex > minDex:
                levels[nearDex][0] = minLevel[0]
            del levels[minDex]
            minHeight = min(level[1] for level in levels)
            for index2, level in enumerate(levels):
                if level[1] == minHeight:
                    minLevel, minDex = level, index2
                    break
